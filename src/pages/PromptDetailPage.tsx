...
import { ArrowLeft, Loader2, MessageSquare, Bot, Edit, Trash2, FileText, Layout } from 'lucide-react';
  ...
  if (!prompt) {
    return (
      <div className="text-center py-20">
        ...
        <Button variant="link" onClick={() => navigate(`/batches/${batchId}`)}>배치로 돌아가기</Button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid gap-6">
        <Tabs defaultValue="text" className="w-full flex flex-col gap-0">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
              ...
              <TabsList variant="line" className="h-8">
                <TabsTrigger value="markdown" className="px-3 py-1 text-xs gap-1">
                  <Layout className="h-3 w-3" />
                  Markdown
                </TabsTrigger>
                <TabsTrigger value="text" className="px-3 py-1 text-xs gap-1">
            ...
            <CardContent>
              <div className="grid min-h-[200px] border rounded-md bg-muted/50 overflow-hidden min-w-0">
                <TabsContent value="markdown" className="mt-0 col-start-1 row-start-1 min-w-0">
                  <div className="p-6 prose prose-sm dark:prose-invert max-w-none h-full break-words">
                    <ReactMarkdown>{prompt.userPrompt}</ReactMarkdown>
                  </div>
                </TabsContent>
                <TabsContent value="text" className="mt-0 col-start-1 row-start-1 min-w-0">
                  <div className="p-6 whitespace-pre-wrap font-mono text-sm h-full break-words">
                    {prompt.userPrompt}
                  </div>
                </TabsContent>
              </div>
        ...
        <Card>
          {prompt.status === 'COMPLETED' ? (
            <Tabs defaultValue="text" className="w-full flex flex-col gap-0">
              ...
              <CardContent>
                <div className="grid min-h-[200px] border rounded-md bg-background shadow-sm overflow-hidden min-w-0">
                  <TabsContent value="markdown" className="mt-0 col-start-1 row-start-1 min-w-0">
                    <div className="p-6 prose prose-sm dark:prose-invert max-w-none h-full break-words">
